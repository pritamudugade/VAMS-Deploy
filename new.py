...
st.set_page_config(page_title="MobiNext-VAMS", layout="wide", page_icon="./favicon-yolo.ico")


# Center-align the titles
st.markdown(
    "<h1 style='text-align: center; font-family: your-font-family;'>MobiNext-VAMS</h1>",
    unsafe_allow_html=True
)
st.markdown(
    "<h2 style='text-align: center; font-family: your-font-family;'>Video Analytics and Monitoring System</h2>",
    unsafe_allow_html=True
)



def process_dual_video_streams(video_file1, video_file2, model1, model2, tracker1=None, tracker2=None, centers1=None,
                               centers2=None):
    results1 = model1.predict(video_file1)
    results2 = model2.predict(video_file2)

    # Process results from the first video stream
    model_name1 = model1.ckpt_path.split('/')[-1].split('.')[0]
    output_folder1 = os.path.join('output_videos', video_file1.split('.')[0])
    if not os.path.exists(output_folder1):
        os.makedirs(output_folder1)
    video_file_name_out1 = os.path.join(output_folder1, f"{video_file1.split('.')[0]}_{model_name1}_output.mp4")
    result_video_json_file1 = os.path.join(output_folder1, f"{video_file1.split('.')[0]}_{model_name1}_output.json")
    json_file1 = open(result_video_json_file1, 'a')
    video_writer1 = cv2.VideoWriter(video_file_name_out1, cv2.VideoWriter_fourcc(*'mp4v'), 30,
                                    (results1[0].orig_img.shape[1], results1[0].orig_img.shape[0]))
    json_file1.write('[\n')

    for result in stqdm(results1, desc=f"Processing video 1"):
        result_list_json = result_to_json(result, tracker=tracker1)
        result_image = view_result(result, result_list_json, centers=centers1)
        video_writer1.write(result_image)
        json.dump(result_list_json, json_file1, indent=2)
        json_file1.write(',\n')
    json_file1.write(']')
    video_writer1.release()

    # Process results from the second video stream
    model_name2 = model2.ckpt_path.split('/')[-1].split('.')[0]
    output_folder2 = os.path.join('output_videos', video_file2.split('.')[0])
    if not os.path.exists(output_folder2):
        os.makedirs(output_folder2)
    video_file_name_out2 = os.path.join(output_folder2, f"{video_file2.split('.')[0]}_{model_name2}_output.mp4")
    result_video_json_file2 = os.path.join(output_folder2, f"{video_file2.split('.')[0]}_{model_name2}_output.json")
    json_file2 = open(result_video_json_file2, 'a')
    video_writer2 = cv2.VideoWriter(video_file_name_out2, cv2.VideoWriter_fourcc(*'mp4v'), 30,
                                    (results2[0].orig_img.shape[1], results2[0].orig_img.shape[0]))
    json_file2.write('[\n')

    for result in stqdm(results2, desc=f"Processing video 2"):
        result_list_json = result_to_json(result, tracker=tracker2)
        result_image = view_result(result, result_list_json, centers=centers2)
        video_writer2.write(result_image)
        json.dump(result_list_json, json_file2, indent=2)
        json_file2.write(',\n')
    json_file2.write(']')
...
# Example usage for dual video streams
video_file1 = st.file_uploader("Upload video 1 for analysis", type=["mp4"])
video_file2 = st.file_uploader("Upload video 2 for analysis", type=["mp4"])

process_dual_video_streams(video_file1, video_file2, model, model, tracker1=DeepSort(max_age=5),
                           tracker2=DeepSort(max_age=5), centers1=[deque(maxlen=30) for _ in range(10000)],
                           centers2=[deque(maxlen=30) for _ in range(10000)])


# create select box for selecting custom object detection model
model_selectbox = st.empty()
model_select = model_selectbox.selectbox("Select Custom Object Detection Model", model_list)
print(f"Selected custom object detection model: {model_select}")
...
# Replace the existing tabs with a dropdown for analytics options
analytics_option = st.selectbox("Select Analytics Option", ["Image Analytics", "Video Analytics", "Live Stream Analytics", "Upload Custom Model"])

if analytics_option == "Image Analytics":
    st.markdown(
        "<h3 style='text-align: center; font-family: your-font-family;'>Image Analytics using Object Detection</h3>",
        unsafe_allow_html=True
    )
    image_file = st.file_uploader("Upload an image for analysis", type=["jpg", "jpeg", "png"])
    process_image_button = st.button("Analyze Image")
    ...
    if image_file is not None and process_image_button:
        ...
        st.image(img, caption="Analyzed Image", channels="BGR")

elif analytics_option == "Video Analytics":
    st.markdown(
        "<h3 style='text-align: center; font-family: your-font-family;'>Video Analytics using Object Detection</h3>",
        unsafe_allow_html=True
    )
    video_file = st.file_uploader("Upload a video for analysis", type=["mp4"])
    process_video_button = st.button("Analyze Video")
    if video_file is None and process_video_button:
        st.warning("Please upload a video for analysis!")
...
elif analytics_option == "Live Stream Analytics":
    st.markdown(
        "<h3 style='text-align: center; font-family: your-font-family;'>Live Stream Analytics using Object Detection</h3>",
        unsafe_allow_html=True
    )
    ...
    run = col_run.button("Start Live Stream Analytics")
    stop = col_stop.button("Stop Live Stream Analytics")
    if stop:
        run = False
    ...
    st.markdown(
        "<h3 style='text-align: center; font-family: your-font-family;'>Upload Custom Object Detection Model</h3>",
        unsafe_allow_html=True
    )
    model_file = st.file_uploader("Upload a custom object detection model (in .pt format):", type=["pt"])
    ...
    if model_file is not None and model_upload_button:
        ...
        if model_name in model_list:
            st.error("The uploaded model already exists in the list!")
        else:
